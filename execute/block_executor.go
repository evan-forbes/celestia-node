...
func BlockExecutorWithMetrics(metrics *state.Metrics) BlockExecutorOption {
...
func (blockExec *BlockExecutor) Store() state.Store {
...
func (blockExec *BlockExecutor) ProcessProposal(
	...
	if err != nil {
		return false, state.ErrInvalidBlock(err)
	}
...
func (blockExec *BlockExecutor) ApplyBlock(
	s state.State, blockID types.BlockID, block *types.Block,
) (state.State, int64, error) {

	if err := validateBlock(s, block); err != nil {
		return s, 0, state.ErrInvalidBlock(err)
	}
	...
	if err != nil {
		return s, 0, state.ErrProxyAppConn(err)
	}
	...
	if err != nil {
		return s, 0, fmt.Errorf("error in validator updates: %v", err)
	}
	...
	s, err = updateState(s, blockID, &block.Header, abciResponses, validatorUpdates)
	if err != nil {
		return s, 0, fmt.Errorf("commit failed for application: %v", err)
	}

	// Lock mempool, commit app state, update mempoool.
	appHash, retainHeight, err := blockExec.Commit(s, block, abciResponses.DeliverTxs)
	if err != nil {
		return s, 0, fmt.Errorf("commit failed for application: %v", err)
	}
	...
	s.AppHash = appHash
	if err := blockExec.store.Save(s); err != nil {
		return s, 0, err
	}
...
func getBeginBlockValidatorInfo(block *types.Block, store state.Store,
...
func updateState(
	...
	if abciResponses.EndBlock.ConsensusParamUpdates != nil {
		...
		nextParams = types.UpdateConsensusParams(s.ConsensusParams, abciResponses.EndBlock.ConsensusParamUpdates)
		err := types.ValidateConsensusParams(nextParams)
		if err != nil {
			return s, fmt.Errorf("error updating consensus params: %v", err)
		}

		s.Version.Consensus.App = nextParams.Version.AppVersion
...
func fireEvents(
	...
	for i, tx := range block.Data.Txs {
		var txHash []byte
		var rawTx []byte
		if origHash, malleatedTx, ismalleated := types.UnwrapMalleatedTx(tx); ismalleated {
			txHash = origHash
			rawTx = malleatedTx
		} else {
			txHash = tx.Hash()
