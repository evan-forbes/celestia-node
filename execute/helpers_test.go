...
func makeAndCommitGoodBlock(
	...
	evidence []types.Evidence) (state.State, types.BlockID, *types.Commit, error) {
	// A good block passes
	s, blockID, err := makeAndApplyGoodBlock(s, height, lastCommit, proposerAddr, blockExec, evidence)
	if err != nil {
		return s, types.BlockID{}, nil, err
	}

	// Simulate a lastCommit for this block from all validators for the next height
	commit, err := makeValidCommit(height, blockID, s.Validators, privVals)
	if err != nil {
		return s, types.BlockID{}, nil, err
	}

	return s, blockID, commit, nil
}

func makeAndApplyGoodBlock(s state.State, height int64, lastCommit *types.Commit, proposerAddr []byte,
	blockExec *BlockExecutor, evidence []types.Evidence) (state.State, types.BlockID, error) {
	block, _ := s.MakeBlock(
		height,
...
func makeState(nVals, height int) (state.State, dbm.DB, map[string]types.PrivValidator) {
...
func makeBlock(s state.State, height int64) *types.Block {
	block, _ := s.MakeBlock(
		height,
		factory.MakeData(makeTxs(s.LastBlockHeight), nil, nil),
		new(types.Commit),
...
func makeHeaderPartsResponsesValPubKeyChange(
	...
	block := makeBlock(s, s.LastBlockHeight+1)
	abciResponses := &tmstate.ABCIResponses{
		BeginBlock: &abci.ResponseBeginBlock{},
		EndBlock:   &abci.ResponseEndBlock{ValidatorUpdates: nil},
	}
	// If the pubkey is new, remove the old and add the new.
	_, val := s.NextValidators.GetByIndex(0)
